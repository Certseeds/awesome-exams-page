...
$$
\begin{aligned}
& \text { and } \iint_0 x \delta d v=\iiint_D x z d v \text { is an odd function of } x \\
& \iiint_0 y \delta d v=\iiint_0 y z d v \text { is an odd function of } y
\end{aligned}
$$
...
$$
z=\left(x^{2}+y^{v}\right)^{2}-1=4-4\left(x^{2}+y^{v}\right) \text{ yields }\left\{\begin{array}{l}
x^{2}+y^{2}=1 \\
z=0
\end{array}\right.
$$
...
$=\iint_{\Sigma} \nabla \times \vec{F} \cdot \vec{n} d \sigma$ 其中 $\Sigma$ 为 $z=1$ 在 $x^{2}+4 y^{2}=z$ 内部的区域.方向为 $(0,0,1)$
