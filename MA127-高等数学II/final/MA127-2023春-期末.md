...
曲面(A) $\int_{0}^{1} \int_{y}^{\sqrt{2 y-y^{2}}} f(x, y) d x d y$

(B) $\int_{0}^{1} \int_{1-\sqrt{1-y^{2}}}^{y} f(x, y) d x d y$

(C) $\int_{0}^{1} \int_{-x}^{x} f(x, y) d y d x+\int_{1}^{2} \int_{-\sqrt{2 x-x^{2}}}^{\sqrt{2 x-x^{2}}} f(x, y) d y d x$

(D) $\int_{0}^{1} \int_{0}^{x} f(x, y) d y d x+\int_{1}^{2} \int_{0}^{\sqrt{2 x-x^{2}}} f(x, y) d y d x$
...
若函数 $y=y(x)$ 的参数方程为 $x=2 t+\ln ^{2} t, y=(t+ \ln t)^{2}, ~ t>0$, 则 $\left.\frac{d^{2} y}{d x^{2}}\right|_{t=1}=$ $\underline{\quad\quad\quad}$

### 2-5

$\int_{0}^{1} \int_{x}^{1} x e^{-y^{3}} d y d x=$ $\underline{\quad\quad\quad}$
...
设曲面 $S$ 是椭圆抛物面 $z=x^{2}+4 y^{2}$ 在平面 $z=1$ 下方的部分, 曲面 $S$ 的内法向量 $n$(即正 $k$ 分量的法向量)为定向, 计算 $\iint_{S} \nabla \times F \cdot n d \sigma$,
