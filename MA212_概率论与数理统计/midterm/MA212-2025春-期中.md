...
一个不透明的袋子中装有5红球和3白球, 每次取1球不放回, 直到取出所有白球为止.设停止时总取球次数为 $X$ , 则 $P(X=6) =$ $\underline{\quad\quad\quad}$ . An opaque bag contains 5 red balls and 3 white balls. Balls are drawn one by one without replacement until all white balls are taken out. Let $X$ be the total number of draws when the process stops. Then, the probability $P(X=6) =$ $\underline{\quad\quad\quad}$
...
设随机变量 $Y$ 服从参数为 1 的指数分布, $a$ 为常数且大于 0 , 则 $P\{Y \geq a+1 \mid Y>a\}=$ $\underline{\quad\quad\quad}$ . Assume the random variable $Y$ follows exponential distribution with the parameter 1. If $a$ is a constant being greater than 0 , then $P\{Y \geq a+1 \mid Y>a\}=$ $\underline{\quad\quad\quad}$

### 2-5

假设离散型随机变量 $X$ 的频率函数为 $P(X=1) =0.6, P(X=0) =0.4$ 且当 $X=1$ 时, $Y \sim U(0,2)$ ；当 $X=0$ 时, $Y \sim \operatorname{Exp}(1)$ .那么 $P(Y \leq 1) =$ $\underline{\quad\quad\quad}$ . Assume that the frequency function of discrete random variable $X$ is $P(X=1) =0.6$ , $P(X=0) =0.4$ . Additionally, we have that if $X=1$ then $Y \sim U(0,2)$ ；and if $X=0$ then $Y \sim \operatorname{Exp}(1)$ . Then $P(Y \leq 1) =$ $\underline{\quad\quad\quad}$

### 2-6

设 $(X, Y)$ 的联合密度为 $f_{X, Y}(x, y) =6 e^{-x} e^{-2 y}(0<x<y<+\infty)$ .则给定 $X=x$ 时, $Y$ 的条件密度为 $f_{Y \mid X}(y \mid x) =$ $\underline{\quad\quad\quad}$ $(y>x)$ . Let $(X, Y)$ have the joint probability density function: $f_{X, Y}(x, y) =6 e^{-x} e^{-2 y}$ for $0<x<y<+\infty$ . Then the conditional density is $f_{Y \mid X}(y \mid x) =$ $\underline{\quad\quad\quad}$ $(y>x)$
...
设 $X$ 与 $Y$ 是两个随机变量, 且 $P\{X \geq 0, Y \geq 0\}=\frac{3}{7}, P\{X \geq 0\}=P\{Y \geq 0\}=\frac{4}{7}$ .则 $P\{\max \{X, Y\} \geq 0\}=$ $\underline{\quad\quad\quad}$ . Assume that $X$ and $Y$ are two random variables, and $P\{X \geq 0, Y \geq 0\}=\frac{3}{7} ; P\{X \geq 0\}=P\{Y \geq 0\}=\frac{4}{7}$ . Then $P\{\max \{X, Y\} \geq 0\}=$ $\underline{\quad\quad\quad}$
