...
Let $A, B, C$ be three mutually independent events, with $P(A) =P(B) =\frac{1}{2}, P(C) =\frac{1}{3}$ . Then the probability that at most one of the events $A, B, C$ happens is $\underline{\quad\quad\quad}$

### 2-2

Distribute randomly 6 chopsticks from 3 different pairs to three persons. Then the probability of at least one person getting two chopsticks from the same pair is $\underline{\quad\quad\quad}$
