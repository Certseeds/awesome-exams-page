1．（1）$\vec{n}_{1}=(1,2,-1) . \quad \overrightarrow{n_{2}}=(1,1,0)$
...
$l_{(x, y) \rightarrow(0,1)} \frac{\ln (1+x y)}{x}=\frac{l}{(x, y) \rightarrow(0,1)} \frac{\ln (1+x y)}{x y} \cdot y \Rightarrow \lim _{y \rightarrow 1} y=1$.
...
日 $\int_{1}^{\infty} \frac{1}{x\left(\ln (t+1+1)^{2}\right.} d x=\int_{1}^{\infty} \frac{d(1+\ln x)}{(1+\ln x)^{2}}$

$\left.=-\frac{1}{1+\ln x}\right]_{1}^{\infty}$
...
&=\sum_{n=1}^{\infty}\left(\frac{1}{2 n}+\frac{(-1)^{n}}{2 n}\right) \\
& \Rightarrow \text { 岁 发 }
\end{aligned}
\end{aligned}
$$
...
& =\sum\left(\frac{(-1)^{n}}{2 n(\ln n+1)}+\frac{1}{2 n(\ln n+1)}\right)
\end{aligned}
$$
...
$\Rightarrow|x|<2$ 时，$\left(\sum_{n=1}^{\infty} a_{n} x^{n}\right)^{\prime}\{$ 促对收做。
...
(3). $\left.\left.\left.v(t)=\int_{0}^{t} a(x) d x+v(0)=\left(e^{x}\right]_{0}^{t}, \frac{x^{2}}{2}\right]_{0}^{t},-\frac{1}{2} \cos 2 x\right]_{i}^{t}\right)$

$$
\begin{aligned}
& a(t)=\left(e^{t}, t, \sin 2 t\right)=\left(e^{t}-1, \frac{t^{2}}{2},-\frac{1}{2} \cos 2 t+\frac{1}{2}\right)+(1,0,0)+(1,0,0) \\
& +(1,0,0) \\
& =\left(e^{+}, \frac{t^{2}}{2}, \frac{1}{2}-\frac{1}{2} \cos 2 t\right) \\
& \Rightarrow \gamma(t)=\int_{0}^{t} v(x) d x+r(0) \\
& =\left(e^{t}-1, \frac{t^{3}}{6}, \frac{t}{2}-\frac{1}{4} \sin 2 t\right) . \\
& \text { (4). } \sin x=x-\frac{x^{3}}{3!}+\cdots \\
& \Rightarrow{\underset{x \rightarrow 0}{ }}_{l_{x \rightarrow 0}} \frac{3 \sin (2 x)-2 \sin (3 x)}{6 x^{3}+x^{4}} \\
& =\lim _{x \rightarrow 0} \frac{3\left(2 x-\frac{(2 x)^{3}}{3!}+0\left(x^{3}\right)\right)-2\left(3 x-\frac{(3 x)^{3}}{3!}+0\left(x^{3}\right)\right)}{6 x^{3}+x^{4}} \\
& =\varliminf_{x \rightarrow 0} \frac{(-4+9) x^{3}+o\left(x^{3}\right)}{6 x^{3}+x^{4}}=\frac{5}{6} \text {. } \\
...
$$
\Rightarrow \sum_{n=3}^{\infty} \frac{\ln n}{n} 4_{2} \Rightarrow \sum_{n=1}^{\infty} \frac{\ln n}{n} 4_{2} .
$$

$\Rightarrow$ 收领城： $0 \leq x<2$ or $[0,2$ ）。

（2）。 $0<0<2$ 时，绝对收佼

而对于 $x=0$ 肘，$\sum_{n=1}^{\infty}(-1)^{n} \frac{\ln n}{n}: \quad \sum_{n=1}^{\infty}\left|\frac{(-1)^{n} \ln n}{n}\right|=\sum_{n=1}^{\infty} \frac{\ln n}{n}$ 发。

$\Rightarrow x=0$ 俶，条件收做。

4．（1） $\lim _{(x, y) \geqslant 10,0)} \frac{y^{2} \sin ^{2} x}{x^{4}+y^{4}}$

Sol．不存在

$$
\begin{aligned}
\text { 令 } y=k x(k \neq 0) . \text { 上戒 } & =\frac{k_{x \rightarrow 0}}{} \frac{k^{2}}{\left(1+k^{4}\right)} \cdot \frac{x^{2} \cdot \sin ^{2} x}{x^{4}} \\
...
$$
0<\frac{C_{n}}{\sqrt{\left(1+a^{n}\right)\left(1+b^{n}\right)}} \leqslant a^{n}
$$

而 $0<a<1, \sum_{n=1}^{\infty} a^{n} 4<\Rightarrow \sum_{n=1}^{\infty} \frac{c_{n}}{\sqrt{\left(1+a^{n}\right)\left(1+b^{n}\right)}} 4<$ ．

$$
\begin{aligned}
& \text { (2) } \\
& \left(\frac{a^{n}}{n^{2}}\right)^{\frac{1}{n}}<\left(\frac{a^{n}}{n^{2}}+\frac{b^{n}}{n}\right)^{\frac{1}{n}} \leqslant\left(\frac{2 a^{n}}{n}\right)^{\frac{1}{n}} \text {. } \\
& \begin{aligned}
\lim _{n \rightarrow \infty}\left(\frac{a^{n}}{n^{2}}\right)^{\frac{1}{n}} & =h_{n \rightarrow \infty} \frac{a}{\left(n^{\frac{1}{n}}\right)^{2}}, h_{n \rightarrow \infty}\left(\frac{2 a^{n}}{n}\right)^{\frac{1}{n}}=l_{n \rightarrow \infty} \frac{2^{\frac{1}{n}} \cdot a}{n^{\frac{1}{n}}}=a . \\
& =a
\end{aligned} \\
& \Rightarrow l_{n \rightarrow \infty}\left(\frac{a^{n}}{n^{2}}+\frac{b^{n}}{n}\right)^{\frac{1}{n}}=a \text {. }
\end{aligned}
$$
...
& \text { 而 } \frac{1}{1+\frac{(x-1)^{2}}{4}}=\sum_{n=0}^{\infty}\left[-\frac{(x-1)^{2}}{4}\right]^{n}=\sum_{n=0}^{\infty} \frac{(-1)^{n}}{4^{n}} \cdot(x-1)^{2 n},\left|\frac{1 x<3}{4}\right|<1 . \\
& \ln [1+(x-1)]=\sum_{n=1}^{\infty} \frac{(-1)^{n-1}(x-1)^{n}}{n},-1 \leqslant x-1<\mid \text { 郎 } 0 \leqslant x<2
\end{aligned}
$$
