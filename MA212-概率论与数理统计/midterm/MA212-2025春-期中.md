...
某种医学检测会给出阳性或阴性结果. 阳性结果旨在表明一个人患有某种(罕见) 疾病, 而阴性结果旨在表明他们没有该疾病. 然而, 假设该检测有时会给出错误的结果: 在没有该疾病的人中, 有 $1 / 100$ 的人会得到阳性检测结果；而在患有该疾病的人中, 有 $2 / 100$ 的人会得到阴性检测结果.如果每 1000 人中有 1 人患有该疾病, 那么一个检测结果为阳性的人实际上患有该疾病的概率是?
...
D. $f_{X}(x) = \begin{cases}x e^{-x}, & x>0, \\ 0, & \text { otherwise.}\end{cases}$
...
设 $A_{1}, A_{2}, A_{3}$ 是样本空间的一个划分且 $P(A_{1}) =0.2, P(A_{2}) =0.5, P(B \mid A_{1}) =0.1$ , $P(B \mid A_{2}) =0.4, P(B \mid A_{3}) =0.7$ .那么 $P(A_{3} \mid B) =$ $\underline{\quad\quad\quad}$

Let $A_{1}, A_{2}, A_{3}$ be a partition of the sample space, with: $P(A_{1}) =0.2, P(A_{2}) =0.5$ , $P(B \mid A_{1}) =0.1, P(B \mid A_{2}) =0.4$ , and $P(B \mid A_{3}) =0.7$ . Then $P(A_{3} \mid B) =$ $\underline{\quad\quad\quad}$

### 2-2

一个不透明的袋子中装有5红球和3白球, 每次取1球不放回, 直到取出所有白球为止.设停止时总取球次数为 $X$ , 则 $P(X=6) =$ $\underline{\quad\quad\quad}$

An opaque bag contains 5 red balls and 3 white balls. Balls are drawn one by one without replacement until all white balls are taken out. Let $X$ be the total number of draws when the process stops. Then, the probability $P(X=6) =$ $\underline{\quad\quad\quad}$
...
设随机变量 $Y$ 服从参数为 1 的指数分布, $a$ 为常数且大于 0 , 则 $P\{Y \geq a+1 \mid Y>a\}=$ $\underline{\quad\quad\quad}$

Assume the random variable $Y$ follows exponential distribution with the parameter 1. If $a$ is a constant being greater than 0 , then $P\{Y \geq a+1 \mid Y>a\}=$ $\underline{\quad\quad\quad}$

### 2-5

假设离散型随机变量 $X$ 的频率函数为 $P(X=1) =0.6, P(X=0) =0.4$ 且当 $X=1$ 时, $Y \sim U(0,2)$ ；当 $X=0$ 时, $Y \sim \operatorname{Exp}(1)$ .那么 $P(Y \leq 1) =$ $\underline{\quad\quad\quad}$

Assume that the frequency function of discrete random variable $X$ is $P(X=1) =0.6$ , $P(X=0) =0.4$ . Additionally, we have that if $X=1$ then $Y \sim U(0,2)$ ；and if $X=0$ then $Y \sim \operatorname{Exp}(1)$ . Then $P(Y \leq 1) =$ $\underline{\quad\quad\quad}$

### 2-6

设 $(X, Y)$ 的联合密度为 $f_{X, Y}(x, y) =6 e^{-x} e^{-2 y}(0<x<y<+\infty)$ .则给定 $X=x$ 时, $Y$ 的条件密度为 $f_{Y \mid X}(y \mid x) =$ $\underline{\quad\quad\quad}$ $(y>x)$

 Let $(X, Y)$ have the joint probability density function: $f_{X, Y}(x, y) =6 e^{-x} e^{-2 y}$ for $0<x<y<+\infty$ . Then the conditional density is $f_{Y \mid X}(y \mid x) =$ $\underline{\quad\quad\quad}$ $(y>x)$
...
设随机变量 $X$ 与 $Y$ 相互独立, $X$ 取值为 1 和 2 的概率均为 $0.5, Y$ 取值为 0 和 1 的概率均为0.5.令 $Z=X+Y$ , 则 $P(Z=2) =$ $\underline{\quad\quad\quad}$ .

### 2-10

设 $X$ 与 $Y$ 是两个随机变量, 且 $P\{X \geq 0, Y \geq 0\}=\frac{3}{7}, P\{X \geq 0\}=P\{Y \geq 0\}=\frac{4}{7}$ .则 $P\{\max \{X, Y\} \geq 0\}=$ $\underline{\quad\quad\quad}$

Assume that $X$ and $Y$ are two random variables, and $P\{X \geq 0, Y \geq 0\}=\frac{3}{7} ; P\{X \geq 0\}=P\{Y \geq 0\}=\frac{4}{7}$ . Then $P\{\max \{X, Y\} \geq 0\}=$ $\underline{\quad\quad\quad}$
...
设 $E$ 和 $F$ 是某试验的样本空间中互不相容的事件, $E$发生概率为$P_1$, $F$ 发生概率为 $P_2$ . 假定重复做该试验至 $E$ 和 $F$ 有一个发生. 这个试验的样本空间是什么? 条件 $E$ 在事件 $F$ 之前发生的概率是多少?

Let $E$ and $F$ be mutually-exclusive events in the sample space of a certain experiment. The probability of $E$ occurring is $P_1$ , and the probability of $F$ occurring is $P_2$ . Suppose the experiment is repeated until either $E$ or $F$ occurs. What is the sample space of this experiment? What is the probability that event $E$ occurs before event $F$ ?
...
假设随机变量 $X$ 和 $Y$ 独立, 且分别服从泊松分布 $X \sim P(\lambda_{1}) , Y \sim P(\lambda_{2})$

(1) 求 $X$ 和 $Y$ 的联合频率函数

(2) 求条件概率 $P(X=k \mid X+Y=n)$ , 其中 $n \geq k$ 是非负整数

Suppose random variables $X$ and $Y$ are independent and follow Poisson distributions $X \sim P(\lambda_{1})$ and $Y \sim P(\lambda_{2})$ , respectively
...
$$
F(x, y) = \begin{cases}c(1-e^{-2 x}) (1-e^{-3 y}) , & 0<x, 0<y \\ 0, & \text { otherwise }\end{cases}
$$
