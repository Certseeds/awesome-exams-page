...
$\alpha>1$ 时, $\forall \varepsilon>0, \exists \delta=\varepsilon^{\frac{1}{\alpha-1}}$, 使得当 $0<\sqrt{x^{2}+y^{2}}<\delta$ 时, $|f^{\alpha-1}(x, y) \sin \frac{1}{x^{2}+y^{2}}| \leq|f^{\alpha-1}(x, y)|=$ $(\max\{|x|,|y|\})^{d-1}<\delta^{\alpha-1}=\varepsilon$
...
$\Rightarrow \lim\limits_{x \rightarrow 0} g(x)$ 不存在
...
对 $\forall x_{0} \in[0, \frac{1}{2}) \cup(\frac{1}{2}, 1]$, 取 $\{S_{n} \in \mathbb{R} \backslash Q \text{ and } S_{n} \neq x_{0}\}$ 满足 $S_{n} \rightarrow x_{0}(n \rightarrow \infty)$, 此时 $f(s_{n})=0$, 取 $\{t_{n} \in \mathbb{Q}$ 且 $t_{n} \neq x_{0}\}$ 满足 $t_{n} \rightarrow x_{0}(n \rightarrow \infty)$, 此时 $f(t_{n})=(t_{n}-\frac{1}{2})^{2}$, 且有

$\lim\limits_{n \rightarrow \infty} f(t_{n})=(x_{0}-\frac{1}{2})^{2}>0$, 因此 $\lim\limits_{x \rightarrow x_{0}} f(x)$ 不存在, $f$ 在点 $x_{0}$ 处不连续
...
由 $F(1,1,2)=0$ 得 $\frac{1}{a^{2}}+\frac{1}{b^{2}}+\frac{4}{c^{2}}=1$

$\Rightarrow k=\frac{1}{6}$

$\Rightarrow a=\sqrt{3}, b=\sqrt{6}, c=2 \sqrt{2}$

## 七

> [!NOTE]
> 这里第一个矩阵内, 右下角的 $\partial f_{3}$ 可能是 $\partial f_{2}$

$J(f \circ g)=J f J g=\left(\begin{array}{lll}\frac{\partial f_{1}}{\partial x} & \frac{\partial f_{1}}{\partial y} & \frac{\partial f_{1}}{\partial z} \\
\frac{\partial f_{2}}{\partial x} & \frac{\partial f_{2}}{\partial y} & \frac{\partial f_{3}}{\partial z}\end{array}\right)
...
$$
\frac{\partial f}{\partial u}(0,0)=\lim\limits_{t \rightarrow 0} \frac{f(t u)}{t}=\cos ^{2} \theta \sin \theta
$$

$\Rightarrow(0,0)$ 处沿任何方向的方向导数皆存在
